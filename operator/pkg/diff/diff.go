// Package diff provide functions that allows to compare set of Kubernetes resources using the logic close to `kubectl diff`.
package diff
...
const (
	...
	syncOptAnnotation           = "kyma-project.io/sync-options"
	ssaAnnotation               = "ServerSideApply=true"
)

// Result holds diffing result of two resources.
type Result struct {
...
// ResultList holds result of two resources sets comparison.
...
func Diff(config, live *unstructured.Unstructured, opts ...Option) (*Result, error) {
	o := applyOptions(opts)
	if config != nil {
	...
	structuredMergeDiff := o.structuredMergeDiff ||
		(config != nil && hasAnnotationOption(config, syncOptAnnotation, ssaAnnotation))
...
func StructuredMergeDiff(config, live *unstructured.Unstructured, gvkParser *managedfields.GvkParser, manager string) (*Result, error) {
...
func structuredMergeDiff(p *SMDParams) (*Result, error) {
	gvk := p.config.GetObjectKind().GroupVersionKind()

	pt := ResolveParseableType(gvk, p.gvkParser)
...
func apply(tvConfig, tvLive *typed.TypedValue, p *SMDParams) (*typed.TypedValue, error) {
	...
	updater := merge.Updater{
		Converter: NewVersionConverter(p.gvkParser, scheme.Scheme, p.config.GroupVersionKind().GroupVersion()),
	}

	// Build a list of managers and which API version they own
	managed, err := DecodeManagedFields(p.live.GetManagedFields())
	if err != nil {
...
func buildDiffResult(predictedBytes []byte, liveBytes []byte) *Result {
	return &Result{
...
func TwoWayDiff(config, live *unstructured.Unstructured) (*Result, error) {
	if live != nil && config != nil {
...
func handleResourceCreateOrDeleteDiff(config, live *unstructured.Unstructured) (*Result, error) {
	if live != nil && config != nil {
	...
	if live != nil {
		...
		return &Result{Modified: false, NormalizedLive: liveData, PredictedLive: []byte("null")}, nil
	} else if config != nil {
		predictedLiveData, err := json.Marshal(config.Object)
		if err != nil {
			return nil, err
		}
		return &Result{Modified: true, NormalizedLive: []byte("null"), PredictedLive: predictedLiveData}, nil
	} else {
...
func generateSchemeDefaultPatch(kubeObj runtime.Object) ([]byte, error) {
	...
	clientgoscheme.Scheme.Default(patched)
	...
	// Ignore empty patch: this only means that clientgoscheme.Scheme.Default(...) made no changes.
...
func applyPatch(liveBytes []byte, patchBytes []byte, newVersionedObject func() (runtime.Object, error)) ([]byte, []byte, error) {
	...
	if err = json.Unmarshal(predictedLiveBytes, &predictedLive); err == nil {
		// 1) Calls 'clientgoscheme.Scheme.Default(predictedLive)' and generates a patch containing the delta of that
		...
		predictedLiveBytes, err = strategicpatch.
			StrategicMergePatch(predictedLiveBytes, patch, predictedLive.DeepCopyObject())
		if err != nil {
			return nil, nil, err
		}
...
func patchDefaultValues(objBytes []byte, obj runtime.Object) ([]byte, error) {
	// 1) Call 'clientgoscheme.Scheme.Default(obj)' to generate a patch containing
...
func ThreeWayDiff(orig, config, live *unstructured.Unstructured) (*Result, error) {
...
func statefulSetWorkaround(orig, live *unstructured.Unstructured) *unstructured.Unstructured {
	...
	_ = unstructured.SetNestedField(live.Object, RemoveListFields(origTemplate, liveTemplate), "spec", "volumeClaimTemplates")
	return live
}
...
func threeWayMergePatch(orig, config, live *unstructured.Unstructured) ([]byte, func() (runtime.Object, error), error) {
	...
	if versionedObject, err := scheme.Scheme.New(orig.GroupVersionKind()); err == nil {
		...
		live = &unstructured.Unstructured{Object: RemoveMapFields(orig.Object, live.Object)}
...
// Array performs a diff on a list of unstructured objects. Objects are expected to match
// environments.
func Array(configArray, liveArray []*unstructured.Unstructured, opts ...Option) (*ResultList, error) {
	numItems := len(configArray)
	if len(liveArray) != numItems {
		return nil, errors.New("left and right arrays have mismatched lengths")
	}

	diffResultList := ResultList{
		Diffs: make([]Result, numItems),
	}
